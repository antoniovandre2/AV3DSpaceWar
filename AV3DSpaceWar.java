    ...
    public void DesenharEspacoDisparo(AV3DSpaceWar comp)
        ...
        if (! (Disparo.equals("")))
            ...
            for (i = 0; i < EspacoLinhas.length; i++)
                ...
                if (((ProdutoEscalaro > 0) || ((ProdutoEscalaro < 0) && ((Phi - Math.PI / 2) % Math.PI > 0) && (zo > z)) || ((ProdutoEscalaro < 0) && ((Phi - Math.PI / 2) % Math.PI < 0) && (zo < z))) && (Math.abs(Math.acos(ProdutoEscalaro / Math.sqrt(xo * xo + yo * yo + zo * zo))) < AnguloVisao + MargemAnguloVisao) && ((ProdutoEscalaro > 0) || ((ProdutoEscalaro < 0) && ((Phi - Math.PI / 2) % Math.PI > 0) && (zd > z)) || ((ProdutoEscalaro < 0) && ((Phi - Math.PI / 2) % Math.PI < 0) && (zd < z))) && ((Math.abs(Math.acos(ProdutoEscalard / Math.sqrt(xd * xd + yd * yd + zd * zd))) < AnguloVisao + MargemAnguloVisao) && (Math.min(xi, Math.min(yi, Math.min(xf, yf))) > 0) && (Math.max(xi + CorrecaoX, xf + CorrecaoX) < TamanhoPlanoX) && (Math.max(yi + CorrecaoY, yf + CorrecaoY) < TamanhoPlanoY)))
                    comp.addLine(xi, yi, xf, yf, CorAlvo);
                }
            }
        }
